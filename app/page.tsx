...
export default function HomePage() {
  ...
  // ★ 修復：移除寫死的滿租假邏輯，直接將區域全名傳給強大的 Search 引擎
  const handleAreaClick = (e: React.MouseEvent, areaName: string) => {
    ...
    setTimeout(() => {
      setLoadingArea(null);
      // 使用提取出的首個關鍵字（如 "大圍 柏傲莊" -> "大圍"）進行廣泛搜尋
      const searchKeyword = areaName.split(' ')[0];
      window.location.href = `/properties?search=${encodeURIComponent(searchKeyword)}`;
    }, 400);
  };
        ...
        <section className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {featuredProps.map((prop: any) => {
              ...
              // ★ 智慧點擊邏輯：有房源去搜尋頁，滿租則彈出候補表單
              const handlePropClick = (e: React.MouseEvent) => {
                if (!prop.hasPublishedRooms) {
                  e.preventDefault();
                  setFullArea(prop.name); // 觸發滿租彈窗
                }
              };

              return (
                <Link 
                  href={`/properties?search=${prop.name}`} 
