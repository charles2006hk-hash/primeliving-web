                  ...
                  <form onSubmit={handleSubmitForm} className="bg-white rounded-2xl p-4 shadow-md border border-slate-200 mt-3 w-full animate-in zoom-in-95 duration-300">
                    ...
                    <div className="space-y-3 h-[300px] overflow-y-auto custom-scrollbar pr-2 pb-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div><label className="block text-[10px] font-bold text-slate-500 mb-1">您的稱呼 *</label><input required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full p-2 border border-slate-200 rounded text-xs text-slate-800 font-bold outline-none focus:border-emerald-500 bg-slate-50" placeholder="如：陳同學" /></div>
                        <div><label className="block text-[10px] font-bold text-slate-500 mb-1">性別</label><select value={formData.gender} onChange={e => setFormData({...formData, gender: e.target.value})} className="w-full p-2 border border-slate-200 rounded text-xs text-slate-800 font-bold outline-none focus:border-emerald-500 bg-slate-50"><option value="未填寫">請選擇</option><option value="男">男</option><option value="女">女</option></select></div>
                      </div>

                      <div className="grid grid-cols-2 gap-2">
                        <div><label className="block text-[10px] font-bold text-slate-500 mb-1">學校/工作地點 *</label><input required value={formData.school} onChange={e => setFormData({...formData, school: e.target.value})} className="w-full p-2 border border-slate-200 rounded text-xs text-slate-800 font-bold outline-none focus:border-emerald-500 bg-slate-50" placeholder="如：香港大學" /></div>
                        <div><label className="block text-[10px] font-bold text-slate-500 mb-1">學位/身份</label><select value={formData.degree} onChange={e => setFormData({...formData, degree: e.target.value})} className="w-full p-2 border border-slate-200 rounded text-xs text-slate-800 font-bold outline-none focus:border-emerald-500 bg-slate-50"><option value="本科 (Undergrad)">本科</option><option value="碩士 (Master)">碩士</option><option value="博士 (PhD)">博士</option><option value="工作人士">工作人士</option></select></div>
                      </div>

                      <div className="grid grid-cols-2 gap-2">
                        <div><label className="block text-[10px] font-bold text-slate-500 mb-1">期望房型</label><select value={formData.roomType} onChange={e => setFormData({...formData, roomType: e.target.value})} className="w-full p-2 border border-slate-200 rounded text-xs text-slate-800 font-bold outline-none focus:border-emerald-500 bg-slate-50"><option value="單人房 (Single)">單人房</option><option value="雙人房 (Shared)">雙人房</option><option value="獨立套房 (Ensuite)">獨立套房</option><option value="整租 (Whole Flat)">整租</option></select></div>
                        <div><label className="block text-[10px] font-bold text-slate-500 mb-1">預計租期</label><select value={formData.duration} onChange={e => setFormData({...formData, duration: e.target.value})} className="w-full p-2 border border-slate-200 rounded text-xs text-slate-800 font-bold outline-none focus:border-emerald-500 bg-slate-50"><option value="6個月內 (短租)">6個月內</option><option value="12個月 (一年死約)">12個月</option><option value="12個月以上">12個月以上</option></select></div>
                      </div>

                      <div className="grid grid-cols-2 gap-2">
                        <div><label className="block text-[10px] font-bold text-slate-500 mb-1">預算 (HKD)</label><select value={formData.budget} onChange={e => setFormData({...formData, budget: e.target.value})} className="w-full p-2 border border-slate-200 rounded text-xs text-slate-800 font-bold outline-none focus:border-emerald-500 bg-slate-50"><option value="6000-9000">6k-9k</option><option value="10000-15000">10k-15k</option><option value="15000以上">15k+</option><option value="custom">自訂</option></select></div>
                        {formData.budget === 'custom' && <div><label className="block text-[10px] font-bold text-emerald-600 mb-1">請輸入預算</label><input required value={formData.customBudget} onChange={e => setFormData({...formData, customBudget: e.target.value})} className="w-full p-2 border border-emerald-300 rounded text-xs text-slate-800 font-bold outline-none bg-emerald-50" placeholder="如: 8500" /></div>}
                      </div>

                      <div className="grid grid-cols-2 gap-2">
                        <div><label className="block text-[10px] font-bold text-slate-500 mb-1">聯絡電話 *</label><input required type="tel" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full p-2 border border-slate-200 rounded text-xs text-slate-800 font-bold outline-none focus:border-emerald-500 bg-slate-50 font-mono" placeholder="含區號" /></div>
                        <div><label className="block text-[10px] font-bold text-slate-500 mb-1">微信 / Email *</label><input required value={formData.contactMethod} onChange={e => setFormData({...formData, contactMethod: e.target.value})} className="w-full p-2 border border-slate-200 rounded text-xs text-slate-800 font-bold outline-none focus:border-emerald-500 bg-slate-50" placeholder="WeChat/Email" /></div>
                      </div>
                      
                      <div><label className="block text-[10px] font-bold text-slate-500 mb-1">其他要求 (選填)</label><textarea rows={2} value={formData.requirements} onChange={e => setFormData({...formData, requirements: e.target.value})} className="w-full p-2 border border-slate-200 rounded text-xs text-slate-800 font-bold outline-none focus:border-emerald-500 bg-slate-50 resize-none" placeholder="如：需有海景、養寵物..." /></div>
                      <div><label className="block text-[10px] font-bold text-purple-600 mb-1">推薦人 (選填)</label><input value={formData.referrer} onChange={e => setFormData({...formData, referrer: e.target.value})} className="w-full p-2 border border-purple-200 rounded text-xs text-slate-800 font-bold outline-none focus:border-purple-500 bg-purple-50" placeholder="佳寓租客/員工姓名" /></div>
                    </div>
        ...
        <div className="p-4 bg-[#f3f4f6] border-t border-slate-200 flex-none pb-8 md:pb-4">
          <form onSubmit={handleSendMessage} className="flex gap-3">
            <input 
              ...
              className="flex-1 px-4 py-3 bg-white border border-slate-200 rounded-full text-sm text-slate-800 font-bold outline-none focus:ring-2 focus:ring-emerald-500/20 transition-shadow shadow-sm disabled:bg-slate-100 disabled:text-slate-400" 
            />
            <button 
              type="submit" 
