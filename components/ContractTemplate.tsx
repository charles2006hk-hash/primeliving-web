  ...
  return (
    <div className="w-[794px] min-h-[1123px] bg-white px-[60px] py-[48px] text-slate-900 font-sans relative shadow-lg origin-top mx-auto select-none">
      ...
      {data.paymentSchedule && data.paymentSchedule.length > 0 && (
        <div className="mb-6">
          <div className="bg-slate-800 text-white px-3 py-1 text-xs font-bold uppercase">Schedule of Payments (合約收費與期數表)</div>
          <table className="w-full text-xs border-collapse border border-slate-300">
            <thead>
              <tr className="bg-slate-100 text-slate-600">
                <th className="border border-slate-300 p-2 text-left">Date (繳期)</th>
                <th className="border border-slate-300 p-2 text-left">Description (項目與期數)</th>
                <th className="border border-slate-300 p-2 text-right">Amount (應繳金額)</th>
              </tr>
            </thead>
            <tbody>
      ...
      {/* --- 核心法律條款摘要 --- */}
      <div className="mb-6 space-y-1.5 text-[11px] text-slate-700 leading-normal border-t border-b border-slate-200 py-3">
      ...
      <div className="grid grid-cols-2 gap-12 pt-6 border-t-2 border-slate-800 text-center relative">
        ...
        <div className="relative pt-8">
          {showStamp && stampUrl && (
            <div className="absolute -top-10 left-1/2 -translate-x-1/2 w-32 h-32 pointer-events-none z-0">
              <img
                src={stampUrl}
                alt="Company Stamp"
